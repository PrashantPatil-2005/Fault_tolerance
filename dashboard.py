...
import plotly.graph_objects as go # Import the graph_objects library
import pandas as pd
import requests
import numpy as np
import json
...
def get_machine_data():
    if OFFLINE_MODE:
        return [{'_id': '6093a75d22b9f000085354b4', 'name': 'Sample Machine (Offline)', 'dataUpdatedTime': '2021-09-02T10:05:46.398Z', 'healthStatus': 'Alarm'}]
    try:
...
def get_bearing_data(machine_id):
    if OFFLINE_MODE:
        return [{'_id': '6093a77822b9f000085354b6', 'name': 'DE Motor Bearing (Offline)', 'healthStatus': 'Alarm'}]
    try:
...
def get_sensor_data(bearing_location_id):
    if OFFLINE_MODE:
        try:
            with open('sample_data.json', 'r') as f: return json.load(f)
...
def get_historical_data(bearing_location_id):
    if OFFLINE_MODE:
        try:
            with open('historical_data.json', 'r') as f: return json.load(f)
        except (FileNotFoundError, json.JSONDecodeError): return None
    return None
...
def update_historical_graph(selected_rows, bearing_data):
    ...
    if not historical_data: return {'data': [], 'layout': {'title': 'No Historical Data Available'}}
...
def update_snapshot_graphs(selected_rows, bearing_data):
    ...
    # --- FIX: Replaced px.line with go.Figure for stability ---
    ...
    raw_fig.update_layout(
        title=f"{graph_title} (Time Domain)",
        xaxis_title="Time (s)",
    ...
    fft_fig.update_layout(
        title=f"{graph_title} (Frequency Spectrum)",
